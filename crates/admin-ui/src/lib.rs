//! ardur-admin — a standalone, read-only observability dashboard over an
...
//! port, configured purely by CLI flags (no env, no shared `Config`), and with
//! **no write path** — every filesystem and Qdrant access is a read. See the
//! crate `README.md` for the security model.
//!
