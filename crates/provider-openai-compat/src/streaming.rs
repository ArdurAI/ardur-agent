//! §3.2b — SSE streaming for the OpenAI-compatible (OpenAI-compatible) backend.
//!
//! OpenAI-compatible follows OpenAI's Chat Completions *streaming* wire format
...
//! - the [`OpenAiCompatChunk`] event the public stream yields and the
...
/// Decode one streamed JSON chunk into zero or more [`OpenAiCompatChunk`]s,
...
fn process_chunk(
    ...
    if let Some(choice) = parsed.choices.into_iter().next() {
        if let Some(content) = choice.delta.content {
            if !content.is_empty() {
                out.push_back(Ok(OpenAiCompatChunk::Content(content)));
            }
        }
...
/// [`OpenAiCompatChunk`] event stream.
///
...
// ---------------------------------------------------------------------------
// §3.X — adapt the OpenAI-compatible-native chunk feed onto the shared `StreamEvent`
// protocol.
// ---------------------------------------------------------------------------

/// Maps the index-keyed [`OpenAiCompatChunk`] feed onto the id-keyed shared
/// [`StreamEvent`] protocol.
///
/// OpenAI/OpenAI-compatible streams tool calls keyed by an integer `index` and carries
...
impl EventAdapter {
    /// Fold one [`OpenAiCompatChunk`] into zero-or-more shared [`StreamEvent`]s.
    fn adapt(
        &mut self,
        chunk: OpenAiCompatChunk,
        out: &mut VecDeque<Result<StreamEvent, ProviderError>>,
    ) {
        match chunk {
            OpenAiCompatChunk::Content(text) => {
                out.push_back(Ok(StreamEvent::ContentDelta(text)));
            }
            OpenAiCompatChunk::ToolCall(delta) => self.adapt_tool_call(delta, out),
            OpenAiCompatChunk::Usage(usage) => out.push_back(Ok(StreamEvent::Usage(usage))),
            OpenAiCompatChunk::Done(reason) => out.push_back(Ok(StreamEvent::Finish(reason))),
        }
    }
...
/// Adapt an [`OpenAiCompatChunk`] stream into the shared [`StreamEvent`] feed
...
where
    S: Stream<Item = Result<OpenAiCompatChunk, ProviderError>> + Send + 'static,
{
...
mod tests {
    ...
    fn content_delta_becomes_content_event() {
        ...
        assert_eq!(events, vec![OpenAiCompatChunk::Content("Hel".to_string())]);
    }
    ...
    fn finish_reason_stop_maps_to_done_stop() {
        ...
        assert_eq!(events, vec![OpenAiCompatChunk::Done(FinishReason::Stop)]);
    }
    ...
    /// Drive a sequence of [`OpenAiCompatChunk`]s through an [`EventAdapter`] and
    /// collect the shared events it emits.
    fn adapt_all(chunks: Vec<OpenAiCompatChunk>) -> Vec<StreamEvent> {
    ...
    fn td(index: u32, id: Option<&str>, name: Option<&str>, args: &str) -> OpenAiCompatChunk {
        OpenAiCompatChunk::ToolCall(ToolCallDelta {
            index,
    ...
    fn adapter_maps_content_usage_and_finish() {
        let events = adapt_all(vec![
            OpenAiCompatChunk::Content("hi".to_string()),
