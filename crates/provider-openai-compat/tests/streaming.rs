//! §12.5 — wiremock round-trips against an OpenAI-compatible SSE streaming
//! endpoint.
//!
...
use ardur_provider_openai_compat::{OpenAiCompatChunk, OpenAiCompatConfig, OpenAiCompatProvider};
...
async fn collect_stream(server: &MockServer, body: String) -> Vec<OpenAiCompatChunk> {
...
async fn parses_content_delta_chunk() {
    ...
    let text: String = chunks
        ...
        .filter_map(|c| match c {
            OpenAiCompatChunk::Content(t) => Some(t.as_str()),
            _ => None,
        })
        .collect();
...
async fn parses_tool_call_delta_assembly() {
    ...
    let done = chunks
        .iter()
        .find_map(|c| match c {
            OpenAiCompatChunk::Done(fr) => Some(fr),
            _ => None,
        })
        .expect("a Done event");
    match done {
    ...
    let frag_count = chunks
        .iter()
        .filter(|c| matches!(c, OpenAiCompatChunk::ToolCall(_)))
        .count();
...
async fn parses_finish_reason_tool_calls() {
    ...
    assert!(
        matches!(done, OpenAiCompatChunk::Done(FinishReason::ToolUse(calls)) if calls.len() == 1),
...
async fn parses_finish_reason_stop() {
    ...
    assert!(
        chunks
            .iter()
            .any(|c| matches!(c, OpenAiCompatChunk::Done(FinishReason::Stop))),
...
async fn parses_usage_on_final_chunk() {
    ...
    let usage = chunks
        .iter()
        .find_map(|c| match c {
            OpenAiCompatChunk::Usage(u) => Some(u),
            _ => None,
        })
...
async fn aborts_on_drop() {
    ...
    assert_eq!(first, OpenAiCompatChunk::Content("first".to_string()));
    drop(stream);
...
/// Live smoke test against the real OpenAI-compatible streaming endpoint.
///
/// Gated on `OPENAI_COMPAT_LIVE_STREAM_TEST=1`, `OPENAI_COMPAT_API_KEY`, and an
/// explicit `OPENAI_COMPAT_LIVE_MODEL`, so CI skips it and the test does not
/// encode a stale model default. Asserts the stream yields some text and a
...
async fn live_stream_smoke() {
    ...
    while let Some(item) = stream.next().await {
        match item.expect("no stream error") {
            OpenAiCompatChunk::Content(t) => text.push_str(&t),
            OpenAiCompatChunk::Done(_) => saw_done = true,
            _ => {}
        }
    }
