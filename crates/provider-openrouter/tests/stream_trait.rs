//! §3.X — the shared [`Provider::stream`] surface over OpenRouter's SSE feed.
//!
//! `Provider::stream` is the uniform streaming method every backend presents;
//! OpenRouter overrides the trait default to adapt its OpenAI-compatible SSE
//! `OpenRouterChunk` feed into shared [`StreamEvent`]s — remapping OpenAI's
...
/// Live smoke test against the real OpenRouter streaming endpoint, via the
/// shared `Provider::stream` surface. Gated on `OPENROUTER_LIVE_STREAM_TEST=1`
/// *and* `OPENROUTER_API_KEY`, so CI (which sets neither) skips it.
#[tokio::test]
