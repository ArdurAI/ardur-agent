...
pub async fn receive_webhook(
    ...
    if let Err(e) = verify_signature(&body, &state.config.secret, signature) {
        warn!("signature verification failed for source={}: {}", state.config.source, e);
    ...
    let event = WebhookEvent::new(EventType::Custom("webhook".to_string()), state.config.source.clone(), payload);
