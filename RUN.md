...
| `codex` (alias `codex-agent`) | OpenAI Codex CLI (ChatGPT subscription, one-shot subprocess wrap) | `CODEX_BINARY` (default: `codex` on `PATH`), `CODEX_DEFAULT_MODEL`, `CODEX_SANDBOX_MODE` (`read-only` \| `workspace-write` \| `danger-full-access`), `CODEX_WORKING_DIR`, `CODEX_TIMEOUT_SECS`, `CODEX_MAX_TOKENS_FLOOR` (default `4096`). Run `codex login` once; no Ardur-side API key |
